...
export default function ResearchTool() {
  ...
  useEffect(() => {
    if (lastJsonMessage) {
      const data = lastJsonMessage as { status: 'agent_message' | 'tool_message' | 'complete', message: string, agent: string, name?: string, content?: string, follow_up_questions?: string[] };
      ...
      if (data.status === "agent_message") {
        ...
        setMessages(prev => [...prev, { role: "tool", content: data.message, search_results: data.name === "Web Search" && data.content ? JSON.parse(data.content) : undefined, tool_name: data.name }]);
